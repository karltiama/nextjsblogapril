...
In this next.js project, we will be using the following libraries:
...
```bash
npm install axios react-icons react-toastify next-auth
```

# Setting Up NextAuth.js

First we need to set up NextAuth.js to handle authentication with Strava.
NextAuth.js is an authentication library for Next.js applications that supports various authentication providers, including OAuth 2.0.

Create a new file called `route.ts` in a folder structure like so `app/api/auth/[...nextauth]` directory with the following content:

```js
import NextAuth from "next-auth";
import StravaProviders from "next-auth/providers";
