...
Historical NBA Data
        |
        v
Postgres / Supabase + S3 Parquet (features)
        |
        v
Backtesting strategy layer
        |
        v
Signals, graded results, reports, manifests
        |
        v
Research dashboard / CLI reports
```
...
Raw NBA API data
        |
        v
Curated player game logs
        |
        v
Feature builder (point-in-time)
        |
        v
S3 Parquet datasets
        |
        v
Backtest runner (DuckDB over local Parquet after download)
        |
        v
Results, manifests, summaries (often back to S3)
```
