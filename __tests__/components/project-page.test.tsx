...
jest.mock("@/content/projects/projectsDataV2", () => ({
	projectsV2: getMockProjects(),
}));
...
describe("ProjectPage", () => {
	it("renders project details correctly", () => {
		const mockProject = getMockProjects().find(
			(p: any) => p.title === "Enduro Stats"
		);
		if (!mockProject) {
			throw new Error("Test project not found");
		}

		const params = Promise.resolve({ projectName: "enduro-stats" });
	...
	it("does not render related blog posts section when no related posts exist", () => {
		const params = Promise.resolve({ projectName: "reel-ratings" });
		render(<ProjectPage params={params} />);
